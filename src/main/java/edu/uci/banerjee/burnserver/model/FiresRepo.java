...
public interface FiresRepo extends JpaRepository<Fire, Integer> {
  ...
  @Query(
      "SELECT f FROM Fire f WHERE (:source is null or f.source = :source) and (:county is null or f.county = :county) and (:countyUnitId is null or f.countyUnitId = :countyUnitId) and"
